...
class SFFNet(nn.Module):
    def __init__(self,
        ...
        self.down = Conv(in_channels=decode_channels, out_channels=decode_channels, kernel_size=1)
    def forward(self, x, y,imagename=None):
        ...
        middleres_x = x
        middleres_y = y
        # print("middleres 的形状:", middleres.shape)
        res1h,res1w = x.size()[-2:]
        xfusefeature_L,xfusefeature_H,xglb,xlocal = self.fuseFeature(middleres_x,imagename)
        yfusefeature_L, yfusefeature_H, yglb, ylocal = self.fuseFeature(middleres_y, imagename)

        xglb = self.MDAF_L(xfusefeature_L,xglb)
        xlocal = self.MDAF_H(xfusefeature_H,xlocal)

        yglb = self.MDAF_L(yfusefeature_L, yglb)
        ylocal = self.MDAF_H(yfusefeature_H, ylocal)

        res_h  = self.WF1(xlocal,ylocal)
        res_l = self.WF1(xglb, yglb)

        middleres_x = self.down(middleres_x)
        middleres_y = self.down(middleres_y)

        res_h = F.interpolate(res_h,size=(res1h,res1w), mode='bicubic', align_corners=False)
        res_l = F.interpolate(res_l, size=(res1h, res1w), mode='bicubic', align_corners=False)

        res_h = middleres_x + res_h
        res_l = middleres_y + res_l

        res_h = self.WF2(res_h,x)
        res_l = self.WF2(res_l, y)
